    ...
    if file_extension == ".dat":
        ...
        for i, row in df.iterrows():
            if df.loc[i, 'dip_x'] > 0 and df.loc[i, 'dip_y'] >= 0:
                theta.append(float(np.arctan(df.loc[i, 'dip_y']/df.loc[i, 'dip_x'])))
            elif df.loc[i, 'dip_x'] > 0 and df.loc[i, 'dip_y'] < 0:
                theta.append(float(np.arctan(df.loc[i, 'dip_y']/df.loc[i, 'dip_x'])) + 2*np.pi)
            elif df.loc[i, 'dip_x'] < 0:
                theta.append(float(np.arctan(df.loc[i, 'dip_y']/df.loc[i, 'dip_x'])) + np.pi)
            elif df.loc[i, 'dip_x'] == 0 and df.loc[i, 'dip_y'] > 0:

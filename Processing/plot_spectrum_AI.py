...
def autocorrelation_chunk(dip_magnitude, start, end, N):
    # Вычисление полной автокорреляционной функции для всего сигнала
    autocorr_full = np.correlate(dip_magnitude, dip_magnitude, mode='full')
    
    # Возвращаем только нужный фрагмент, который соответствует сдвигу от start до end
    return autocorr_full[len(dip_magnitude)-1 + start: len(dip_magnitude)-1 + end]
            
def calculate_autocorrelation(dip_magnitude):
    N = len(dip_magnitude)
    num_cores = cpu_count()
    
    # Определение границ для каждой задачи
    chunk_size = N // num_cores
    ranges = [(i*chunk_size, (i+1)*chunk_size if i != num_cores-1 else N) for i in range(num_cores)]
    
    with Pool(processes=num_cores) as pool:
        # Параллельное вычисление автокорреляции
        results = pool.starmap(autocorrelation_chunk, [(dip_magnitude, start, end, N) for start, end in ranges])
    
    # Объединение всех фрагментов в один массив
    autocorr = np.concatenate(results)
    return autocorr
    ...
    for address, dirs, names in os.walk(directory):
        for name in names:
            ...
            if file_extension == ".dat":
                ...
                dip_magnitude_corr = calculate_autocorrelation(dip_magnitude)

    ...
    if os.path.isdir(folder) == True:     
        ...
        for i in files:
            ...
            if file_extension == ".dat":
                ...
                df = df.where(df['Amplitude'].max()&df['Frequency']>3000)
                # frequency.append(df['Frequency'].where(df['Amplitude'].max()&df['Frequency']>3000))
                # amplitude.append(df['Amplitude'].where(df['Frequency']>3000).max())
        # exp = pd.DataFrame(list(zip(frequency, amplitude)), columns =['Frequency', 'Spectral Density (a.u.)'])
        # print(exp)
